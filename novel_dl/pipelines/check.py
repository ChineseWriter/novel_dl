...
class CheckPipeline:
    ...
    HASH_LEN    = 64   # 书籍哈希值的长度, 用于检查 book_hash 字段
    CONTENT_LEN = 800  # 章节内容的最小长度, 用于检查章节内容的完整性
    ...
    def process_item(
        ...
        若检查失败(下级方法抛出 AssertionError),
        则记录警告日志并抛出 DropItem 异常丢弃该 item.
        若 item 类型不属于上述两者, 则直接丢弃并记录警告日志.

        ## 参数
        - item: 待处理的 item 实例, 类型为 BookItem 或 ChapterItem.
        - spider: 当前处理 item 的爬虫实例, 类型为 GeneralSpider 的子类.
        """
    ...
    @staticmethod
    def check_book_item(item: BookItem, _: GeneralSpider):
        """
        # check_book_item
        ...
        - _: GeneralSpider 的子类的实例, 可用于记录日志或其他操作.
        ...
        if item["other_info"] is None: item["other_info"] = {}
        if item["cover_urls"] is None: item["cover_urls"] = []
        if item["covers"]     is None: item["covers"]     = []
    ...
    @staticmethod
    def check_chapter_item(item: ChapterItem, spider: GeneralSpider):
        """
        # check_chapter_item
        ...
        if "other_info"  not in item:   item["other_info"]  = {}
        if item["update_time"] is None: item["update_time"] = 0.0
        if item["other_info"]  is None: item["other_info"]  = {}
        ...
        # 逻辑检查
        assert len(item["book_hash"]) == CheckPipeline.HASH_LEN, \
            "书籍哈希字符值必须为 64 位"
        assert item["index"]        >= 0,   "章节索引不能为负数"
        assert item["update_time"]  >= 0.0, "章节更新时间不能为负数"

        # 其它检查
        if len(item["content"]) <= CheckPipeline.CONTENT_LEN:
            spider.logger.warning(
